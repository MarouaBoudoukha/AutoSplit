...
import { client, schemaConfig, contract } from "./client.mjs";
...
export async function createExpenseOnChain(
  amount,
  description,
  participants,
  shares,
  ...
  try {
    ...
    console.error("Error while creating the expense on the blockchain:", error);
    throw error;
  }
}

export async function createGroupOnChain(name, members) {
  try {
    ...
    console.error("Error while creating the group on the blockchain:", error);
    throw error;
  }
}

export async function updateExpenseWithAttestation(
  expenseId,
  attestationId,
  schemaId,
  indexingValue,
  ...
  try {
    ...
    await tx.wait();
    console.log(`Expense ${expenseId} updated with attestation information.`);
  } catch (error) {
    console.error(
    ...
    console.log(
      `Timestamp: ${
        attestation.attestTimestamp
          ? new Date(parseInt(attestation.attestTimestamp) * 1000).toISOString()
          : "N/A"
      }`,
    );
